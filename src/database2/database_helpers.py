...
from src.database2.schema.balance_sheet import BalanceSheetItemStandardizedORM, BalanceSheetAtEndOfPeriodORM, BalanceSheetAtEndOfPeriodYearStartORM
...
def save_balance_sheet_year_start_to_db(
    company_name: str,
    stock_code: str,
    balance_sheet: BaseModel,
):
    ...
    balance_sheet_data = {
        balance_sheet_item_dict[item.code]: item.amount_beginning_of_year
        for item in balance_sheet.balance_sheet_items
        if item.code in balance_sheet_item_dict
    }
    ...
    try:
        existing_record = db.query(BalanceSheetAtEndOfPeriodYearStartORM).filter(
            BalanceSheetAtEndOfPeriodYearStartORM.period_end_date == balance_sheet.period_end_date,
            (BalanceSheetAtEndOfPeriodYearStartORM.company_name == company_name) | (BalanceSheetAtEndOfPeriodYearStartORM.stock_code == stock_code)
        ).first()
        ...
        if existing_record:
            ...
            db_balance_sheet = BalanceSheetAtEndOfPeriodYearStartORM(**balance_sheet_data)
            db.add(db_balance_sheet)
            db.commit()
            db.refresh(db_balance_sheet)
            print(f"Saved balance sheet year start for {company_name} as of {balance_sheet.period_end_date}")
            return db_balance_sheet

    except SQLAlchemyError as e:
        db.rollback()
        print(f"Failed to save balance sheet year start: {e}")
        return None
